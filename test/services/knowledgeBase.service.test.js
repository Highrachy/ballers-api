...
describe('KnowledgeBase Service', () => {
  describe('#getPostInKnowledgeBaseById', () => {
    ...
    before(async () => {
      await addPostToKnowledgeBase(KnowledgeBaseFactory.build({ _id: id, updatedBy: id }));
    });
  ...
  describe('#addPostToKnowledgeBase', () => {
    let countedPosts;
    const id = mongoose.Types.ObjectId();
    const post = KnowledgeBaseFactory.build({ updatedBy: id });
    ...
    context('when an invalid data is entered', () => {
      it('throws an error', async () => {
        try {
          const InvalidPost = KnowledgeBaseFactory.build({ title: '', updatedBy: id });
  ...
  describe('#updatePostInKnowledgeBase', () => {
    ...
    before(async () => {
      await addPostToKnowledgeBase(KnowledgeBaseFactory.build({ _id, updatedBy: _id }));
    });
